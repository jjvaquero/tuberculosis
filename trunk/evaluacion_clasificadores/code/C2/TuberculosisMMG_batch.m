...
pathname = '.\929_Positive_binarized_edited';
tresf= dir(pathname);
num = numel(tresf);
b= cell(num,1);
...
%ileName,pathname]=uigetfile('*.bmp','Pick the samples bmp-files','MultiSelect', 'on');%Abre ventana de dialogo
if isequal(fileName,0)|isequal(pathname,0)
    ...
    %[fileName,pathname]=uigetfile('*.bmp;*.tif','Pick one or more bmp or tiff files');%Abre ventana de dialogo
    
    directorios = 'C:\img';
    
    direc= dir(directorios);
    num2 = numel(direc);
    ...
    for lt= 3:numel(z)
        ...
        for li= 3:numel(b)
            ...
            if ~isempty(strfind(fileName,'.bmp') )
            ...
            %0.001:20.5150,0.0005:22.1053,0.0001:25.7448        
                    ...
                    for k=1:1
                        ...
                        %Añadido de Chema:
                        image = tophat_filter(image, ones(9,9));
                ...
                resultados = [resultados bacilos];    
            end %isequal(fileName,0)|isequal(pathname,0)
        end  %~isempty(strfind(fileName,'.bmp') )
        
        fprintf('%i ', resultados)
        fprintf('\n', resultados)
    end
end
