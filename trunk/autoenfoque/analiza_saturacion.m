function void = analiza_saturacion
...
file_saturacion = fopen(sprintf('%s/saturacion.txt', PATH_DATA), 'w');
saturacion = uint32(zeros(300, 1));
...
for i=3:length(dir_list),
    ...
    for j=1:20,
        ...
        maximo = max(max(orig_img_green));
        
        if (maximo > saturacion(i-2))
            saturacion(i-2) = maximo;
        end           
