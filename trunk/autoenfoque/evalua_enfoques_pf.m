function void = evalua_enfoques_pf(path)
...
for i=3:length(dir_list),
    ...
    for j=3:22,
        ...
        tic;
        ObjectSizes = calSizesTiles(orig_img_green,100,400,0);
        if max(ObjectSizes)>80                
           orig_img_green_pf = tophat_filter(orig_img_green, mask);
        else orig_img_green_pf = orig_img_green;
        end
        tiempo_pf = tiempo_pf + toc;
        ...
        file_th = sprintf('%s/%s/%s-th.tif', PATH_DATA, dir_list(i).name, ...
            base_nombre);
        file_pf = sprintf('%s/%s/%s-pf.tif', PATH_DATA, dir_list(i).name, ...
            base_nombre);
        ...
        imwrite(orig_img_green_pf, file_pf); 
    ...
    fprintf(tiempo_file, ...
        '%G\t%G\t%G\t%G\n', tiempo_m2, tiempo_m4, ...
        tiempo_m8, tiempo_th, tiempo_pf);
    fclose(tiempo_file);
    
    t_total = tiempo_m2 + tiempo_m4 + tiempo_m8 + tiempo_th + tiempo_pf;
    ...
    for funcion=1:length(funciones_eval),
        ...
        [sth, tth] = enfoca(funcion, 4, nextfile_str, ...
            base_nombre, funciones_eval);
        [spf, tpf] = enfoca(funcion, 5, nextfile_str, ...
            base_nombre, funciones_eval);
...
filtro_lista = {'orig', 'm2', 'm4', 'm8', 'th', 'pf'};
